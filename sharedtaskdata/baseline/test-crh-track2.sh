...
python $ONMTTEST -model models/$DATA.model_step_10000.pt -src onmt-data/$DATA-src-test.txt -output onmt-data/$DATA-src-test.txt.out -replace_unk -verbose -gpu 0 -n_best 10 -beam 10 > onmt-data/$DATA-src-test.txt.nbest.out
